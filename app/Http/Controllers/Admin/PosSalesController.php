...
class PosSalesController extends Controller
{
    ...
    public function searchUsers(Request $request)
    {
        ...
        $users = ClientUser::where('name', 'like', "%{$query}%")
            ->orWhere('email', 'like', "%{$query}%")
            ->take(5)
            ->get(['id', 'name', 'email']);

            \Log::info('SearchUsers query', ['query'=>$query, 'count'=>$users->count()]);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($validated['items'] as $itemData) {
                ...
                // Atualizar o estoque
                $estoqueAnterior = $vinyl->stock;
                $vinyl->stock -= $itemData['quantity'];
                $vinyl->in_stock = ($vinyl->stock > 0); // Atualiza in_stock com base no estoque restante
                $vinyl->save();
                
                // Log para debug
                \Illuminate\Support\Facades\Log::info('Atualizando estoque do disco ID: ' . $vinyl->id, [
                    'estoque_anterior' => $estoqueAnterior,
                    'quantidade_vendida' => $itemData['quantity'],
                    'estoque_atual' => $vinyl->stock,
                    'in_stock' => $vinyl->in_stock
                ]);
            }
