...
<div class="border rounded p-3 mb-3 bg-gray-50">
    ...
    @if($order->shipping_label_url)
        ...
        <div class="flex flex-col gap-2 mb-2">
            ...
            <form action="{{ route('admin.orders.generate-shipping-label', $order->id) }}" method="POST" class="inline">
                @csrf
                <button type="submit" class="px-4 py-2 border border-blue-600 text-blue-600 rounded hover:bg-blue-50 flex items-center justify-center w-full">
    ...
    @elseif($statusValue == 'payment_approved' || $statusValue == 'preparing' || $statusValue == 'shipped')
        ...
        <form action="{{ route('admin.orders.generate-shipping-label', $order->id) }}" method="POST" class="inline">
            @csrf
            <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 flex items-center justify-center w-full">
