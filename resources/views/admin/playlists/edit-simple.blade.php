            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                @if($playlist->is_dj)
                    ...
                    <div class="mt-6">
                        ...
                        @if($playlist->has_dj_photo)
                            <!-- Foto atual -->
                            <div class="mb-4">
                                <p class="text-sm text-gray-600 mb-2">Foto atual:</p>
                                <div class="relative inline-block">
                                    <img src="{{ $playlist->dj_photo_url }}" alt="Foto atual do DJ" class="h-32 w-32 object-cover rounded-lg border">
                                </div>
                                <div class="mt-2">
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600">
                                    <label for="dj_photo" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                        <span>{{ $playlist->has_dj_photo ? 'Alterar foto' : 'Enviar foto' }}</span>
                ...
                <div class="bg-white rounded-lg shadow-sm border p-6">
                    ...
                    <div class="space-y-4" id="tracks-container">
                        @for($i = 0; $i < 10; $i++)
                            @php
                                $existingTrack = $playlist->tracks->where('position', $i + 1)->first();
                                $selectedValue = old("tracks.{$i}", $existingTrack ? $existingTrack->product_id : '');
                            @endphp
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const existingTracks = @json($playlist->tracks->pluck('product_id', 'position')->map(function($productId, $position) {
        return ['position' => $position - 1, 'product_id' => $productId];
    })->values());

    // Initialize each track selector
    for (let i = 0; i < 10; i++) {
        const existingTrack = existingTracks.find(t => t.position === i);
        const selectedValue = existingTrack ? existingTrack.product_id : '';
        initTrackSelector(i, products, selectedValue);
    }
