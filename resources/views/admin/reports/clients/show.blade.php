...
<div class="container px-6 mx-auto grid">
    ...
    <div class="flex justify-between items-center">
        ...
        <div class="flex space-x-2">
            ...
            @if($client->cart && $client->cart->products->count() > 0 && $client->cart->updated_at <= now()->subDays(7))
                <form method="POST" action="{{ route('admin.reports.clients.send_abandoned_cart_email', $client->id) }}" class="inline">
                    @csrf
                    <button type="submit" 
                            class="px-4 py-2 text-sm text-orange-600 bg-orange-100 rounded-lg hover:bg-orange-200 transition-colors duration-150"
                            onclick="return confirm('Tem certeza que deseja enviar o email de carrinho abandonado para {{ $client->email }}?')">
                        <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-1.1 5M7 13l-1.1-5m0 0L5.4 5M7 13l4.5-4.5M16 13v6a2 2 0 01-2 2H6a2 2 0 01-2-2v-6"></path>
                        </svg>
                        Enviar Email Carrinho
                    </button>
                </form>
            @endif
        </div>
